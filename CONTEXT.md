...
### 2026-06-19 · goal-13l datadriven-hub
- **Turned the Data-Driven Modeling section into the flagship showcase** (docs-only; `git diff --stat origin/main -- brainmass/` **empty**). Replaced the `data_driven/index.ipynb` stub with the narrative hub + a CURATED guided path, and fleshed out `data_driven/roadmap.md`'s three deferred homes. Only the 2 owned files changed (`index.ipynb` +282, `roadmap.md` +149/-27); no toctree edits (the landing `index.rst` already toctrees `data_driven/index` under "Showcase"; the hub's own `{toctree}` keeps `roadmap` in nav).
- **Hub `index.ipynb` (8 cells, executes exit 0)**: headline narrative (differentiable + data-driven IS brainmass's headline), the construct/fit/train "three verbs" table, the conservative **6-row brainmass-vs-TVB-vs-neurolib** table (byte-consistent with `index.rst` + `why_differentiable`; links to `/concepts/why_differentiable` for the deep dive rather than re-deriving), a **runnable teaser** (canonical Hopf settled-amplitude grad fit: derive target amp from a true `a=1.5` model, fit a fresh `Param(0.1, fit=True)` node back via `Fitter(..., loss_fn=...)` — **recovered a=1.500, loss 2.18e-11** at lr=0.1/120 steps), then a 5-stage guided path (Understand→Learn→Apply→See→Extend) linking every curated docname.
- **⚠ The teaser must converge tightly or the narrative ("walks straight to the value") lies.** Hopf amplitude ∝ √a is flat near a=1.5, so 60 Adam steps @lr=0.05 from a=0.1 only reached a=1.403 (loss 1.5e-3). Bumped to **lr=0.1, n_steps=120 → a=1.500, loss 2e-11**. Also derive the target from a *true model* (`target_amp = settled_amplitude(make_node(1.5))`) instead of a magic amplitude number, so "recovered a vs true a" reads honestly. Reconfirms 13c/13e: fit a SCALAR SUMMARY (settled RMS amplitude with a non-zero `init_x=Constant(0.5)` kick), never the phase-degenerate time series.

...
import brainmass
import brainstate
from brainstate.nn import Param, Const, ReluT
...
class JansenRitNetwork(brainstate.nn.Module):
    ...
    def __init__(
        ...
        self.dynamics = brainmass.JansenRitTR(
            ...
            k=Param(5.5, t=ReluT(0.5)),
            w_ll=Param(np.full((node_size, node_size), 0.05, dtype=dtype)),
            w_ff=Param(np.full((node_size, node_size), 0.05, dtype=dtype)),
            w_bb=Param(np.full((node_size, node_size), 0.05, dtype=dtype)),
            g_l=Const(400),
            g_f=Const(10),
            g_b=Const(10),
...
def train_hdeeg_jr():
    scope = (100, 900)
    lm, sc, dist, activities, inputs = load_subject_eeg_data(scope=scope)
